        ...
        for (int j = 0; j < sqrtSamples; j++) {
            ...
            if (groundDist > 0.0) {
                ...
                if (dot(pos, sunDir) > 0.0) {
                    ...
                    lum += transmittance*groundAlbedo*getValFromTLUT(TransmittanceLUT, tLUTRes.xy, hitPos, sunDir);
                }
            }
...
layout (location = 0) out vec4 bMultiScattering;

void main()
{
    if (TexCoords.x * resolution.x >= (msLUTRes.x + 1.5) || TexCoords.y * resolution.y >= (msLUTRes.y + 1.5)) {
        bMultiScattering = vec4(0.0, 0.0, 0.0, 1.0);
        return;
    }

    float u = clamp(TexCoords.x * resolution.x, 0.0, msLUTRes.x - 1.0) / msLUTRes.x;
    float v = clamp(TexCoords.y * resolution.y, 0.0, msLUTRes.y - 1.0) / msLUTRes.y;
